...
class ConceptBank:
    def __init__(self, concept_dict, device):
        ...
        for k, (tensor, _, _, intercept, margin_info) in concept_dict.items():
            ...
            all_vectors.append(torch.tensor(tensor,requires_grad=False).float().view(1,-1).to(device))
        ...
        self.concept_info.vectors = torch.cat(all_vectors,dim=0)
        print (self.concept_info.vectors.shape)
